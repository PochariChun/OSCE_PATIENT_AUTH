# src/python/embedding/build_miniLM_embedding_service.py
"""
paraphrase-multilingual-MiniLM-L12-v2 + FAISS）

- 用法:
    python src/python/embedding/build_miniLM_embedding_service.py --data-path src/lib/rag_lookup_data_cleaned.jsonl --output-index src/lib/faiss_index
    python src/python/embedding/build_miniLM_embedding_service.py --rebuild-index

- 職責：建立並儲存 FAISS 向量索引。

- 嵌入模型：paraphrase-multilingual-MiniLM-L12-v2

- 資料結構：儲存 faiss.index, doc_map.pkl, documents.pkl

- 設計: FAISS + MiniLM-L12ing 分數，再混合 Jaccard+Levenshtein。

- 流程: launch_miniLM_embedding_api > launch_miniLM_embedding_api.py
...
def query_index(index, doc_map, documents, query_text, top_k=5):
    # 查詢預處理
    query_text = clean_text(normalize_query(query_text))
