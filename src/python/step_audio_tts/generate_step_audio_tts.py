...
"""
使用 Step-Audio-TTS-3B 模型为 dialogue_answers.json 中的每个回答生成语音
并将结果保存到 /audio 目录
"""
...
SAMPLE_RATE = 24000  # Step-Audio-TTS 的采样率

def download_model():
    """
    下载并加载 Step-Audio-TTS-3B 模型
    """
    try:
        from transformers import AutoProcessor, AutoModel
        
        print(f"正在加载 {MODEL_ID} 模型...")
        # 使用 trust_remote_code=True 允许执行模型的自定义代码
        processor = AutoProcessor.from_pretrained(MODEL_ID, trust_remote_code=True)
        # 使用 AutoModel 而不是 AutoModelForTextToWaveform
...
def generate_audio(text, model, processor, voice_preset="female", emotion="neutral"):
    """
    使用 Step-Audio-TTS-3B 生成语音
    
    Args:
        text: 要转换为语音的文本
        model: 加载的模型
        processor: 文本处理器
        voice_preset: 语音预设 (female, male)
        emotion: 情感类型 (neutral, happy, sad)
            ...
            for i, item in enumerate(category_answers):
                ...
                try:
                    ...
                    if category == "condition":
                        emotion = "sad"  # 病情询问使用悲伤情感
                    elif category == "instruction":
                        emotion = "happy"  # 护理指导使用开心情感
...
if __name__ == "__main__":
    ...
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='使用 Step-Audio-TTS-3B 生成语音')
